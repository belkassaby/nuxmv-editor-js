...
export const WALKTHROUGHS: Walkthrough[] = [
    ...
    },
    {
        ...
        steps: [
            ...
            { text: 'Code changes: with "Propose quick fixes" ticked (and optionally an LLM chosen on the first screen), findings come with a proposed change. Quick fixes cover missing switch cases (listed explicitly, behaviour kept), writes after an await (a re-check just before), and resources acquired twice or never released (release first, add dispose). Each change is applied in memory and every check is run again: ✓ verified means the finding is gone and nothing new appeared.' },
