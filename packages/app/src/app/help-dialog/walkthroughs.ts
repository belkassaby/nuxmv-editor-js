...
export const WALKTHROUGHS: Walkthrough[] = [
    ...
    },
    {
        ...
        steps: [
            ...
            { text: 'The results open in the same window: findings by category with their fix and counterexample (errors and warnings; tick notes to see more), the models, the design patterns found and the paradigm profile of each part. Open model loads a model into the editor and checks it, so the counterexample is one click away; Download report saves the Markdown report.' },
            { text: 'The same from a terminal (TypeScript/JavaScript, Angular templates and Python). With NUXMV_PATH set the models are checked by nuXmv; without it, by an explicit-state checker for the standard properties.', code: 'NUXMV_PATH=/path/to/nuXmv npx pflow extract path/to/project' },
