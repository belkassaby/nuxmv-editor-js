...
export const GLOSSARY: GlossaryTerm[] = [
    ...
    { term: 'Typestate', aka: 'resource lifecycle', definition: 'The states an object goes through for a resource it holds (idle, held, released, disposed) and the operations allowed in each. pflow extract checks that a held resource is never lost: acquired again or dropped before being released.' },
    { term: 'Code model extraction', aka: 'model extraction, specification mining', definition: 'Building a model of existing code from its source: here, state machines from fields with finite types, lifecycles from acquire/release calls, contracts of design patterns and the layer graph. The model is an abstraction: a counterexample is confirmed by replaying it on the code.' },
    { term: 'Abstraction', aka: 'over-approximation', definition: 'A model with more behaviours than the code (conditions it cannot read are dropped). What it proves for all its runs holds for the code; a counterexample may be spurious and is checked on the real code.' },
