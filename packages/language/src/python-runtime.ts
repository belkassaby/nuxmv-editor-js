...
class EditorLink:
    """Streams every state change to a running provenflow (POST /api/live/<channel>).
...
class StateMachine:
    ...
    * Runtime monitors re-check the monitorable properties on every step.
    ...
    def record_to(self, path):
        """Append every step to a JSON Lines file, for trace conformance checking in the editor (Trace -> Check a recorded run) or with "pflow conform"."""
        import json
        import time
    ...
    def add_nurv_monitor(self, module):
        """Attach a full-LTL monitor generated by NuRV (provenflow: Python tab -> NuRV monitors,
        or "pflow nurv"). Its verdict is "unknown" until the observed run decides the property for
    ...
    def link_editor(self, url="http://127.0.0.1:3000", channel="default", commands=False):
        """Stream state changes to a running provenflow. With commands=True the editor can also
    ...
    def enable_tracing(self, tracer=None):
        ...
        tracer = tracer or trace.get_tracer("provenflow." + DIAGRAM["name"])
