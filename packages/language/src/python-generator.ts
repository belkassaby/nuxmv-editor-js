...
export async function generatePython(model: DiagramModel, options: { sourceName?: string } = {}): Promise<GeneratedPython> {
    ...
    const lines: string[] = [];
    lines.push(`"""${name}: state machine generated by ProvenFlow${options.sourceName ? ` from ${options.sourceName}` : ''}.`);
    lines.push('');
    ...
    lines.push('    fsm.link_editor(channel="me") # live state in provenflow');
    lines.push('"""');
    lines.push('');
